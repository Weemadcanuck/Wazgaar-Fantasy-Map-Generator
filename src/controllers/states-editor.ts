...
const LEGEND_NAME = "Polities"; // the legend box this editor toggles
...
function renderDialog(): void {
  ...
  const editorHtml = /* html */ `<div id="${dialogId}" class="dialog stable editorDialog">
    ...
    <div id="statesBottom" class="editorToolbar">
      ...
      <button id="statesAnnex" data-tip="Annex polities: click the annexing polity, then the polities it absorbs. Hold Shift to keep annexing" class="icon-crown"></button>
...
function renderStatesPage(view: TableView<State>): void {
  ...
  for (const s of view.rows) {
    ...
    >
      ...
      ${Notes.getIcon("this polity")}
      <span data-col="locate" data-tip="Locate the polity" class="icon-target"></span>
      <span data-col="focus" data-tip="Toggle polity focus" class="icon-pin ${focused ? "" : " inactive"}"></span>
      <span data-col="lock" data-tip="Lock the polity to protect it from re-generation" class="icon-lock${
        s.lock ? "" : "-open"
      }"></span>
      <span data-col="remove" data-tip="Remove the polity" class="icon-trash-empty"></span>
    </div>`;
  }
...
function toggleLegend(): void {
  ...
  if (!data.length) return void tip("No polities to show", false, "error");
  ...
  alertMessage.innerHTML = /* html */ `
    <form id='mergeStatesForm' style="overflow: hidden; display: flex; flex-direction: column; gap: 1em;">
      <p style="margin:0">
        ...
        Hover over a row to highlight the polity on the map.
      </p>
...
function confirmStatesMerge(statesToMerge: number[], rulingStateId: number, onConfirm?: () => void): void {
  ...
  confirmationDialog({
    ...
    // prettier-ignore
    message: /* html */ `
      <p>The following polities will be <strong>removed</strong>: ${statesToMerge.map(stateId => `${stateEmblem(stateId)}${(pack.states)[stateId].name}`).join(", ")}.</p>
      <p>Removed polities data (settlements, provinces, regiments) will be assigned to ${stateEmblem(rulingState.i)}${rulingState.name}.</p>
...
function downloadStatesCsv(): void {
  ...
  const name = `${getFileName("Polities")}.csv`;
