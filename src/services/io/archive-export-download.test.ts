...
beforeEach(() => {
  ...
  vi.stubGlobal("options", {
    map: { lore: { name: "Jotun" }, units: { population: { scale: 1000, urbanization: { rate: 1 } } } }
  });
  vi.stubGlobal("mapHistory", [{ created: 1788160068282 }]);
...
describe("in-application Archive export", () => {
  ...
  it("sends the same export plan through the desktop bridge", async () => {
    ...
    expect(loadArchiveExportDiagnostic(localStorage, 1788160068282)).toMatchObject({
      status: "written",
      target: "directory",
      worldId: "jotun-live"
    });
