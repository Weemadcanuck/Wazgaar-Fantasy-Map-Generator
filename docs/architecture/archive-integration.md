...
Export is additive and preserves the meaning of existing FMG fields. The 1.154.0 integration keeps upstream journeys at `.map` slot 52 and appends a labelled `azgaar-obsidian` extension at slot 53. The extension carries custom layers, the Archive world ID and original legacy notes. Older fork saves are migrated by `src/services/io/fork-data.ts`.

The current fork implements the pure export plan, a full-JSON command-line writer, an in-application Archive export profile, a `.zip` download, and guarded desktop-folder export. The application creates a stable world ID, stores it in the map extension and retains a local map-ID fallback. Export profiles are also stored per map and default to reference-safe geography. Optional generated population, economy, military, and diplomacy snapshots must be explicitly selected. Desktop-folder export uses a native directory picker, shows a file-level dry-run summary, blocks unmanaged or author-edited targets, asks for confirmation, and writes the manifest last. Managed-block updates and reverse synchronization remain unimplemented.
...
- Reinterpreting upstream save fields; fork data uses additive fields and its own versioned extension.
